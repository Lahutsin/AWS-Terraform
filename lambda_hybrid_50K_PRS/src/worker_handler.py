...
def _detect_lane(record):
    source_arn = record.get("eventSourceARN", "")
    queue_name = source_arn.rsplit(":", 1)[-1]

    if queue_name.endswith("-priority"):
        return "priority"
    if queue_name.endswith("-bulk"):
        return "bulk"
    return "buffer"


def _store_payload(request_id, payload, lane):
    timestamp = datetime.now(timezone.utc).isoformat()
    object_key = f"async/{lane}/{request_id}.json"
    ...
    s3_client.put_object(
        ...
        Metadata={"request-id": request_id, "lane": lane},
    )
    ...
    table.update_item(
        ...
        UpdateExpression=(
            ...
            "lane = :lane, stack_name = :stack_name"
        ),
...
def _claim_request(request_id, timestamp, lane):
    try:
        table.put_item(
            Item={
                "id": request_id,
...
def handler(event, _context):
    ...
    for record in event.get("Records", []):
        ...
        lane = _detect_lane(record)

        try:
            payload = _parse_message(record)
            request_id = payload.get("request_id") or request_id

            if not _claim_request(request_id, datetime.now(timezone.utc).isoformat(), lane):
                continue

            _store_payload(request_id, payload, lane)
